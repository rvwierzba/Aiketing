...
const ImageGenerationPage = () => {
  ...
  useEffect(() => {
    // A proteção de rota agora é mais simples graças ao nosso AuthContext
    if (isLoggedIn === false) {
      router.push('/login');
    }
  }, [isLoggedIn, router]);
      ...
      <main className="flex-grow p-4 md:p-8">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-3xl font-bold text-brand-dark mb-2">
            {t('ImageGeneratorPage.Title')}
          </h1>
          <p className="text-brand-text mb-8">
            {t('ImageGeneratorPage.Subtitle')}
          </p>
              ...
              <div className="space-y-4">
                <div>
                  <label htmlFor="prompt" className="block text-sm font-medium text-gray-700">
                    {t('ImageGeneratorPage.PromptLabel')}
                  </label>
                  <textarea
                    ...
                    placeholder={t('ImageGeneratorPage.PromptPlaceholder')}
                    value={prompt}
...
export const getStaticProps = async (context: any) => {
  return {
    props: {
      ...(await getLocaleProps(context, ['Header', 'Footer', 'ImageGeneratorPage'])),
    },
  };
}
